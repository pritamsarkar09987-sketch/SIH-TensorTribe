	...
	for {
		...
		case frame := <-h.broadcast:
			...
			for client := range h.clients {
				...
				default:
					// Selective Backpressure: remote client buffer is full due to degraded network.
					// Drop oldest stale frame so client receives the latest real-time frame
					// without blocking upstream AI ingestion or disconnecting the client.
					select {
					case <-client.send:
					default:
					}
