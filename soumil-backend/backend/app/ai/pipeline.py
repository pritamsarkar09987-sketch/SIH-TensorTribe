            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if now - last_log_time >= 10.0:
                        last_log_time = now
                        print(f"[AI Streamer] Live streaming active -> {self.broadcaster_url} ({frames_sent} frames sent)")
                else:
                    now = time.time()
                    if now - last_log_time >= 5.0:
                        last_log_time = now
                        print(f"[AI Streamer Warning] Ingest endpoint returned HTTP {res.status_code} ({self.broadcaster_url})")
            except Exception as e:
                now = time.time()
                if now - last_log_time >= 5.0:
                    last_log_time = now
                    print(f"[AI Streamer Warning] Frame upload to {self.broadcaster_url} failed: {e}")
